...
it('can add the star name and star symbol properly', async() => {
    ...
    let user2 = accounts[2];
    let starId = 9;
    await instance.createStar('Saie', starId, {from: user2});

    let name = await instance.name();
    let symbol = await instance.symbol();
    let starLookUp = await instance.lookUptokenIdToStarInfo(starId);

    assert.equal(name,'Saie Kurakula Token');
    assert.equal(symbol,'SKT');
    assert.equal(starLookUp,'Saie');
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user2 = accounts[2];
    let starId = 34;
    await instance.createStar('star1', starId, {from: user2});
