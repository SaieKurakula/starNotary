...
contract StarNotary is ERC721 {
    ...
    string public constant name = "Saie Kurakula Token";
    ...
    // function to check if a star exists
    function starExists (uint256 _tokenId) public view returns (bool){
      for (uint i; i< SDK_Tokens.length;i++){
          if (SDK_Tokens[i] == _tokenId )
          return true;
      }
    }
    ...
    function lookUptokenIdToStarInfo (uint _tokenId) public view returns (string memory) {
        ...
        require(starExists(_tokenId), "Please Send a Valid Star.");
